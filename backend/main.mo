...
actor {
    ...
    var walletAddresses : OrderedMap.Map<Principal, [Text]> = principalMap.empty();

    public shared ({ caller }) func addWalletAddress(address : Text) : async WalletOperationResult<()> {
        // Authorization check: Only authenticated users can add wallet addresses
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
            return #error(#unauthorizedAccess);
        };

        let currentAddresses = switch (principalMap.get(walletAddresses, caller)) {
            case null { [] };
            case (?addresses) { addresses };
        };

        if (currentAddresses.size() >= 9) {
            return #error(#walletLimitReached);
        };

        let updatedAddresses = Array.append(currentAddresses, [address]);
        walletAddresses := principalMap.put(walletAddresses, caller, updatedAddresses);

        #success(());
    };

    public query ({ caller }) func getWalletAddresses() : async [Text] {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
            Debug.trap("Unauthorized: Only users can view wallet addresses");
        };

        switch (principalMap.get(walletAddresses, caller)) {
            case null { [] };
            case (?addresses) { addresses };
        };
    };
    ...
    public shared ({ caller }) func createWallet(_walletLabel : ?Text) : async WalletOperationResult<WalletCreationResult> {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
            return #error(#unauthorizedAccess);
        };

        let currentWalletCount = switch (walletOwnerMap.get(userWallets, caller)) {
            case null { 0 };
            case (?userWalletMap) {
                var count = 0;
                for ((_, _) in walletMap.entries(userWalletMap)) {
                    count += 1;
                };
                count;
            };
        ...
        let updatedUserWalletMap = switch (walletOwnerMap.get(userWallets, caller)) {
            case null {
                walletMap.put(walletMap.empty(), nextWalletId, newWallet);
            };
            case (?existingMap) {
                walletMap.put(existingMap, nextWalletId, newWallet);
            };
        };
        ...
        #success({
            ...
            recoveryPhrase = "This is a not so secret recoveryPhrase";
            chain = #evm;
        });

    };

    public query ({ caller }) func listWallets() : async WalletOperationResult<[EpiqWallet]> {
        ...
        switch (walletOwnerMap.get(userWallets, caller)) {
            case null { #success([]) };
            case (?userWalletMap) {
                let walletCount = Iter.size(walletMap.entries(userWalletMap));
                if (walletCount > 9) {
                    return #error(#walletLimitReached);
                };

                #success(Iter.toArray(walletMap.vals(userWalletMap)));
            };
        };
    };
